...
module.exports = {
    createCSS: function (document, styles, sheet) {
        ...
        styleNode.setAttribute('type', 'text/less');
        if (sheet.media) {
...
module.exports = function(options, logger) {
    ...
    FileManager.prototype.doXHR = function doXHR(url, type, callback, errback) {
        ...
        if (typeof xhr.overrideMimeType === 'function') {
            xhr.overrideMimeType('text/less');
        }
        logger.debug('XHR: Getting \'' + url + '\'');
        xhr.open('GET', url, async);
        xhr.setRequestHeader('Accept', type || 'text/x-less, text/less; q=0.9, */*; q=0.5');
        xhr.send(null);
...
module.exports = function() {
    return {
        ...
        /* Allows you to add a path to every generated import and url in your less.
         * This does not affect less import statements that are processed, just ones
         * that are left in the output less. */
        rootpath: '',

        /* By default URLs are kept as-is, so if you import a file in a sub-directory 
         * that references an image, exactly the same URL will be output in the less.
...
module.exports = function(SourceMapBuilder) {
    ...
    ParseTree.prototype.toCSS = function(options) {
        ...
        try {
            ...
            if (compress) {
                logger.warn('The compress option has been deprecated. We recommend you use a dedicated less minifier, for instance see less-plugin-clean-less.');
            }
...
Ruleset.prototype.eval = function (context) {
    ...
    // Evaluate everything else
    for (i = 0; (rule = rsRules[i]); i++) {
        // for rulesets, check if it is a less guard and can be removed
...
// lets you call a less selector with a guard
            ...
            for (targetExtendIndex = 0; targetExtendIndex < extendsListTarget.length; targetExtendIndex++) {
                ...
                if (matches.length) {
                    ...
                    extend.selfSelectors.forEach(function(selfSelector) {
                        ...
                        // we know that any others will be duplicates in terms of what is added to the less
...
ImportVisitor.prototype = {
    ...
    processImportNode: function(importNode, context, importParent) {
        ...
        try {
            ...
            // attempt to eval properly and treat as less
            importNode.css = true;
        ...
        if (evaldImportNode && (!evaldImportNode.css || inlineCSS)) {
            ...
            // try appending if we haven't determined if it is less or not
...
var visitors = {
    ...
    ToCSSVisitor: require('./to-less-visitor')
};
...
ToCSSVisitor.prototype = {
    ...
    visitAtRuleWithoutBody: function(atRuleNode, visitArgs) {
        ...
        if (atRuleNode.name === '@charset') {
            ...
            // be considered illegal less as it has to be on the first line
            if (this.charset) {
