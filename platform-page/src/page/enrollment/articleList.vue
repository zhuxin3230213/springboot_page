...
<template>
  <el-container class="info-edit-container">
    ...
    <el-main>
      <data-grid ref="datagrid" url="infoContent/listContents" auth v-bind="$attrs" :buttons="buttons" :actions="actions" :postParams="postParams" @addBtnClick="add" @removeBtnClick="remove" @editActionClick="edit" @removeActionClick="({row})=>remove({rows:[row]})" :pageSize="10">
...
export default {
      ...
      this.$refs.form.validate(success => {
        if (success) {
          ...
          articleApi
            ...
            .then(data => {
              this.$refs.datagrid.reload(false)
              this.dialogVisible = false
              this.clearData()
            })
            .catch(error => {
  ...
  beforeMount () {
    const parentCode = this.$props.params.parentCode
    this.postParams = {
      moduleName: parentCode
    }
